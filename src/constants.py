...
# Type aliases for clarity (similar to C stdint.h)
uint64_t = int
uint32_t = int
uint16_t = int
uint8_t = int
...
# e_ident[EI_MAG0:EI_MAG3+1] in the ELF header
class EI_MAG(IntEnum):
...
# e_ident[EI_VERSION] in the ELF header
...
class EI_OSABI(IntEnum):
    ...
    """
    ELFOSABI_RESERVED_MIN = 64
    ELFOSABI_RESERVED_MAX = 255
...
class E_TYPE(IntEnum):
    ...
    """
    FreeBSD S4 image specific
    """
    ET_FREEBSD_S4_IMAGE = ET_LOOS + 1  # FreeBSD S4 image
...
class P_TYPE(IntEnum):
    ...
    FreeBSD S4 image specific
    """
    PT_FREEBSD_S4_PCB        = 0x61000054
    PT_FREEBSD_S4_TRAMPOLINE = 0x61000055


class P_FLAG(IntEnum):
    PF_X        = 0x1         # Execute
    PF_W        = 0x2         # Write
    PF_R        = 0x4         # Read
...
TRAMPOLINE_DEFAULT_PATH = "trampoline/hello_world.bin"
S4_IMG_DEFAULT_OUTPUT_PATH = "bin/s4img"
PT_LOAD_DEFAULT_CONTENT = 0XA5